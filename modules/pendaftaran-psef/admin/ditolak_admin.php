    ...
    <div class="row">
        ...
        <div class="col-7 align-self-center">
            <div class="d-flex no-block justify-content-end align-items-center" id="refresh-page">
                <button onclick="routing('ditolak_admin')" type="button" class="btn waves-effect waves-light btn-rounded btn-primary"><i class="fas fa-redo"></i> Segarkan Halaman</button>
            </div>
        </div>
    </div>
</div>
...
<script id="view-data" type="text/x-handlebars-template">
    ...
    <form class="m-t-30">
        ...
        <button type="button" class="btn btn-danger" onclick="routing('ditolak_admin')">Kembali</button>
    </form>
</script>
...
<script>
    ...
    function view_data_detail(data_permohonan, data_apotek, data_pemohon, data_klinik, data_rs){
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function (datax, textStatus, xhr) {
                let tot_history_ditolak = datax.value.length
                let cek_current_status = datax.value[tot_history_ditolak-2].statusId
