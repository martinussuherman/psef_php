...
<script id="view-data" type="text/x-handlebars-template">
    ...
    <div class="col-sm-12">
        ...
        <button type="button" class="btn btn-danger" onclick="routing('pending_kasubdit')">Kembali</button>
...
<script>
    ...
    function reject_data(id){
        Swal.mixin({
            ...
            if (result.value) {
                $.ajax({
                    url: url_api_x+'Permohonan/KepalaSubDirektoratKembalikan',
                    type: 'POST',
    ...
    function process_data(id){
        ...
        swal({
            ...
            if (result.value) {
                $.ajax({
                    url: url_api_x+'Permohonan/KepalaSubDirektoratSetujui',
                    type: 'POST',
                    beforeSend: function (xhr) {
                        xhr.setRequestHeader('Authorization', 'Bearer '+accesstoken+'');
                    },
